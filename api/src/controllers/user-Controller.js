...
export class UserController {
    ...
    static async createUser(req, res) {
        ...
        try {
            ...
            if (checkUsername) return res.status(400).json({ message: 'El nombre de usuario ya existe' })

            const checkUser = await User.findOne({ email })
            if (checkUser) return res.status(400).json({ message: 'El email ya existe' })
    ...
    static async login(req, res) {
        ...
        try {
            ...
            const checkUser = await user.findOne({ email })
            if (!checkUser) return res.status(404).json({ error: 'El usuario no existe' })
